...
Write-Host "
PLEASE READ `n
MoneyPow is a basic implementation of 'crash', a popular gambling game where a number is generated on a curve, weighted toward 1, this number is then used as a multiplier for a users bet. I tested the formula used here by generating 1,000,000 points in python to verify a high degree of accuracy relative to the industry (as I'm using a non standard method of calculation). I found the accuracy to be exceptional, as shown by this histogram I made(source code provided here too) of the 1,000,000 points: `nhttps://imgur.com/a/cgHGiCo `n`nNormally the crash point (also called the 'multiplier' in this project) is generated by parsing sha256 hashes, however that's beyond the scope of this game. For this project I will use a derived formula, shown here https://youtu.be/F1HA7e3acSI at 03:10 `nTo play the game you will input an amount to bet, then you will be asked for the 'multiplier'. (Type checking is supported!) The multiplier you give, must be below the generated crash point in order to win. For example, if you set your multiplier to 3, then the generated crash point is 4, you will receive 3x your bet (Calculated, per standard, as: balance = balance-bet+bet*multiplier), and if the crash point were 2, then you would have received nothing, and lost your entire bet because the generated point was below your 'expected' point. So the fun is making the call as to what crash point is better for your current balance, and bet size, in order to make money! (Of course, the house always wins! (discussed further in the linked video)). By default everyone starts with `$1500. `n`nHave fun!"
